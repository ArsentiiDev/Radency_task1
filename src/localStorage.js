...
export function getFromLocalStorage() {
  ...
  for (let archive in archivesobj) {
    if (archivesobj[archive] != 0) {
      console.log(archivesobj[archive]);
      if (archive in sumListItems) {
        let sumlist = sumList.getElementsByClassName(archive)[0];
        let target = sumlist.getElementsByClassName("archiveCount")[0];
        target.innerHTML = archivesobj[archive];
        let count = sumlist.getElementsByClassName("count")[0];
        count.innerHTML = count.innerHTML - archivesobj[archive];
      } else {
        console.log("ehre");
